...
import { ConnectorClient, ConnectorError, type ResearchEvidence } from './connector'
...
type Screen = 'home' | 'create' | 'manual-create' | 'proposals' | 'tree'
...
function operationErrorMessage(error: unknown, tr: (key: string) => string) {
  if (error instanceof ConnectorError && error.code === 'native_research_required') return 'CodexのWeb検索を確認できなかったため、結果を保存せず処理を中止しました。'
  if (error instanceof ConnectorError && error.code === 'invalid_output') return tr('error.invalidOutput')
...
export default function App() {
  ...
  const connector = useMemo(() => new ConnectorClient(settings.connectorUrl), [settings.connectorUrl])
  ...
  const createChallenge = async (node: NodeRecord) => {
    ...
    try {
      ...
      if ((workspaceEpochRef.current[projectId] ?? 0) !== operationEpoch) throw new ConnectorError('The learning tree was replaced or deleted while the challenge was being generated.', 'stale_operation')
  ...
  const mergeExpansion = (base: Workspace, nodeId: string, expansion: Awaited<ReturnType<ConnectorClient['nodeExpand']>>, branchCount: number): Workspace => {
  ...
  const submitAnswer = async (node: NodeRecord, challenge: Challenge, answer: string) => {
    ...
    try {
      ...
      if ((workspaceEpochRef.current[projectId] ?? 0) !== operationEpoch) throw new ConnectorError('The learning tree was replaced or deleted while the answer was being graded.', 'stale_operation')
      ...
      if (gradedNode?.children.length === 0 && connectorStatus.connected && connectorStatus.authenticated) {
        ...
        try {
          ...
          if ((workspaceEpochRef.current[projectId] ?? 0) !== operationEpoch) throw new ConnectorError('The learning tree was replaced or deleted while branches were being generated. The answer was saved.', 'stale_operation')
  ...
  const expandNode = async (node: NodeRecord, branchCount = 3) => {
    ...
    try {
      ...
      if ((workspaceEpochRef.current[projectId] ?? 0) !== operationEpoch) throw new ConnectorError('The learning tree was replaced or deleted while branches were being generated.', 'stale_operation')
  ...
  if (screen === 'create') return <><CreateScreen lang={lang} tr={tr} form={createForm} setForm={setCreateForm} busy={busy === 'propose'} error={error} onSubmit={handlePropose} onCancel={goHome} connectorStatus={connectorStatus} />{busy === 'propose' && <div className="create-progress-floating"><GradingLog items={gradingLog} label="ツリーを作成中" /></div>}</>
  if (screen === 'manual-create') return <><ManualTreeScreen lang={lang} tr={tr} form={manualForm} setForm={setManualForm} busy={busy === 'manual-create'} error={error} onSubmit={handleManualCreate} onCancel={goHome} connectorStatus={connectorStatus} /></>
  ...
  return <>
    <HomeScreen workspaces={workspaces} lang={lang} tr={tr} connectorStatus={connectorStatus} error={error} onNew={() => { setError(''); setScreen('create') }} onManual={() => { setError(''); setScreen('manual-create') }} onOpen={openWorkspace} onImport={() => fileInputRef.current?.click()} onExport={exportWorkspace} onSettings={() => setSettingsOpen(true)} onHelp={() => setHelpOpen(true)} onRefreshConnector={refresh} onDelete={deleteProject} />
...
function HomeScreen({ workspaces, lang, tr, connectorStatus, error, onNew, onManual, onOpen, onImport, onExport, onSettings, onHelp, onRefreshConnector, onDelete }: { workspaces: Workspace[]; lang: UiLanguage; tr: (key: string, vars?: Record<string, string | number>) => string; connectorStatus: ConnectorStatus; error: string; onNew: () => void; onManual: () => void; onOpen: (id: string) => void; onImport: () => void; onExport: () => void; onSettings: () => void; onHelp: () => void; onRefreshConnector: () => void; onDelete: (id: string) => void }) {
  return <div className="app-frame home-frame"><Header lang={lang} tr={tr} connectorStatus={connectorStatus} onImport={onImport} onExport={onExport} onSettings={onSettings} />
    <main className="home-content">
      ...
      <div className="home-actions"><button className="primary-button" onClick={onNew}><Icon name="plus" /> {tr('home.newTree')}</button><button className="secondary-button" onClick={onManual}><Icon name="branch" /> {tr('home.manualTree')}</button><p className="form-note">{tr('home.workspaceSaveHint')}</p></div>
...
function CreateScreen({ lang, tr, form, setForm, busy, error, onSubmit, onCancel, connectorStatus }: { lang: UiLanguage; tr: (key: string, vars?: Record<string, string | number>) => string; form: { topic: string; goal: string; researchMode: 'ja' | 'global'; challengeMode: ChallengeMode; priorKnowledge: string }; setForm: (form: { topic: string; goal: string; researchMode: 'ja' | 'global'; challengeMode: ChallengeMode; priorKnowledge: string }) => void; busy: boolean; error: string; onSubmit: (event: FormEvent) => void; onCancel: () => void; connectorStatus: ConnectorStatus }) {
  return <div className="app-frame"><Header lang={lang} tr={tr} connectorStatus={connectorStatus} onBack={onCancel} /><main className="form-shell"><button className="back-link" onClick={onCancel}><Icon name="back" /> {tr('nav.back')}</button><section className="form-intro"><p className="eyebrow">{tr('create.eyebrow')}</p><h1>{tr('create.title')}</h1><p>{tr('create.description')}</p></section>{error && <ErrorBanner message={error} />}<form className="tree-form" onSubmit={onSubmit}><label>{tr('create.topic')}<input autoFocus value={form.topic} onChange={(event) => setForm({ ...form, topic: event.target.value })} placeholder={tr('create.topicPlaceholder')} /></label><label>{tr('create.goal')}<textarea rows={4} value={form.goal} onChange={(event) => setForm({ ...form, goal: event.target.value })} placeholder={tr('create.goalPlaceholder')} /></label><fieldset><legend>{tr('create.researchMode')}</legend><div className="mode-options"><label className={`mode-option ${form.researchMode === 'ja' ? 'selected' : ''}`}><input type="radio" name="research-mode" checked={form.researchMode === 'ja'} onChange={() => setForm({ ...form, researchMode: 'ja' })} /><span><strong>{tr('create.ja')}</strong><small>{tr('create.jaHint')}</small></span></label><label className={`mode-option ${form.researchMode === 'global' ? 'selected' : ''}`}><input type="radio" name="research-mode" checked={form.researchMode === 'global'} onChange={() => setForm({ ...form, researchMode: 'global' })} /><span><strong>{tr('create.global')}</strong><small>{tr('create.globalHint')}</small></span></label></div></fieldset><ChallengeModePicker lang={lang} value={form.challengeMode} onChange={(challengeMode) => setForm({ ...form, challengeMode })} /><details className="advanced-options"><summary>{tr('create.advanced')}<small>{tr('create.advancedHint')}</small></summary><label>{tr('create.prior')}<select value={form.priorKnowledge} onChange={(event) => setForm({ ...form, priorKnowledge: event.target.value })}><option value="auto">{tr('create.auto')}</option><option value="beginner">{tr('create.beginner')}</option><option value="intermediate">{tr('create.intermediate')}</option><option value="advanced">{tr('create.advancedLevel')}</option></select></label></details><div className="form-actions"><button type="button" className="secondary-button" onClick={onCancel}>{tr('create.cancel')}</button><button className="primary-button" disabled={busy || !connectorStatus.connected || !connectorStatus.authenticated}>{busy ? tr('proposal.loading') : tr('create.submit')}</button></div>{!connectorStatus.connected && <p className="form-note"><span className="status-dot muted" /> {tr('home.connectHint')}</p>}</form></main></div>
}

function ManualTreeScreen({ lang, tr, form, setForm, busy, error, onSubmit, onCancel, connectorStatus }: { lang: UiLanguage; tr: (key: string, vars?: Record<string, string | number>) => string; form: ManualTreeForm; setForm: (form: ManualTreeForm) => void; busy: boolean; error: string; onSubmit: (event: FormEvent) => void; onCancel: () => void; connectorStatus: ConnectorStatus }) {
  const update = (key: keyof typeof form, value: string) => setForm({ ...form, [key]: value })
  return <div className="app-frame"><Header lang={lang} tr={tr} connectorStatus={connectorStatus} onBack={onCancel} /><main className="form-shell"><button className="back-link" onClick={onCancel}><Icon name="back" /> {tr('nav.back')}</button><section className="form-intro"><p className="eyebrow">{tr('manual.eyebrow')}</p><h1>{tr('manual.title')}</h1><p>{tr('manual.description')}</p></section>{error && <ErrorBanner message={error} />}<form className="tree-form" onSubmit={onSubmit}><label>{tr('manual.treeTitle')}<input autoFocus value={form.title} onChange={(event) => update('title', event.target.value)} /></label><label>{tr('create.topic')}<input value={form.topic} onChange={(event) => update('topic', event.target.value)} /></label><label>{tr('create.goal')}<textarea rows={3} value={form.goal} onChange={(event) => update('goal', event.target.value)} /></label><fieldset><legend>{tr('create.researchMode')}</legend><div className="mode-options"><label className={`mode-option ${form.researchMode === 'ja' ? 'selected' : ''}`}><input type="radio" name="manual-research-mode" checked={form.researchMode === 'ja'} onChange={() => setForm({ ...form, researchMode: 'ja' })} /><span><strong>{tr('create.ja')}</strong><small>{tr('create.jaHint')}</small></span></label><label className={`mode-option ${form.researchMode === 'global' ? 'selected' : ''}`}><input type="radio" name="manual-research-mode" checked={form.researchMode === 'global'} onChange={() => setForm({ ...form, researchMode: 'global' })} /><span><strong>{tr('create.global')}</strong><small>{tr('create.globalHint')}</small></span></label></div></fieldset><ChallengeModePicker lang={lang} value={form.challengeMode} onChange={(challengeMode) => setForm({ ...form, challengeMode })} /><details className="advanced-options" open><summary>{tr('manual.context')}</summary><label>{tr('manual.philosophy')}<textarea rows={2} value={form.philosophy} onChange={(event) => update('philosophy', event.target.value)} /></label><label>{tr('manual.learner')}<textarea rows={2} value={form.learnerProfile} onChange={(event) => update('learnerProfile', event.target.value)} /></label><label>{tr('manual.branches')}<textarea rows={2} value={form.branches} onChange={(event) => update('branches', event.target.value)} placeholder={tr('manual.listHint')} /></label><label>{tr('manual.advantages')}<textarea rows={2} value={form.advantages} onChange={(event) => update('advantages', event.target.value)} placeholder={tr('manual.listHint')} /></label><label>{tr('manual.tradeoffs')}<textarea rows={2} value={form.tradeoffs} onChange={(event) => update('tradeoffs', event.target.value)} /></label></details><div className="form-actions"><button type="button" className="secondary-button" onClick={onCancel}>{tr('create.cancel')}</button><button className="primary-button" disabled={busy || !connectorStatus.connected || !connectorStatus.authenticated}>{busy ? tr('proposal.loading') : tr('manual.submit')}</button></div>{!connectorStatus.connected && <p className="form-note"><span className="status-dot muted" /> {tr('home.connectHint')}</p>}</form></main></div>
}
...
function GradeReview({ grade, tr, workspace, onRetry, onOpenNode }: { grade: LastGrade; tr: (key: string, vars?: Record<string, string | number>) => string; workspace: Workspace; onRetry: () => void; onOpenNode: (id: string) => void }) {
  ...
  const next = response.recommendedNodeIds.find((id) => workspace.tree.nodes[id])
  return <div className="grade-review"><div className="grade-header"><div className={`grade-score grade-${response.grade}`}>{response.grade}</div><div><p className="eyebrow">{tr('grade.title')}</p><h3>{label}</h3></div><strong className="xp-earned">{tr('grade.xpEarned', { xp: grade.xpAwarded })}</strong></div><ReviewList title={tr('grade.summary')} items={[response.summary]} plain /><ReviewList title={tr('grade.correct')} items={response.correct} icon="check" /><ReviewList title={tr('grade.missing')} items={response.missing} icon="minus" /><ReviewList title={tr('grade.misconceptions')} items={response.misconceptions.length ? response.misconceptions : [tr('grade.noItems')]} icon="warn" /><ReviewList title={tr('grade.nuance')} items={response.nuance} /><ReviewList title={tr('grade.next')} items={[response.nextStep ?? '']} plain />{grade.unlockedIds.length > 0 && <div className="unlock-note"><Icon name="branch" /><span><strong>{tr('grade.unlocked')}</strong><small>{grade.unlockedIds.map((id) => workspace.tree.nodes[id]?.title).filter(Boolean).join(' · ')}</small></span></div>}<div className="grade-actions"><button className="secondary-button" onClick={onRetry}>{tr('grade.retry')}</button>{next && <button className="text-button" onClick={() => onOpenNode(next)}>{tr('grade.recommended')} <Icon name="arrow" /></button>}</div><AttemptHistory attempts={workspace.attempts.filter((item) => item.nodeId === grade.nodeId)} tr={tr} /></div>
}
